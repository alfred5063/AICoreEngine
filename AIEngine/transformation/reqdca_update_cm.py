...
def update_cm(withpath_df, reqdca_base):
  try:
    ...
    i = 0
    for i in range(len(withpath_df)):

      # Create empty main dataframe
      main_df = pd.DataFrame(columns=['Adm. No', 'File No.', 'Disbursement Listing', 'Disbursement Claims', '''Member's Name''', 'Policy No', 'Bill No',
                                      'Adm. Date', 'Dis. Date', 'Hospital', 'Diagnosis', 'Insurance', 'Patient', 'Total', 'Submission Date', 'OB Received Date',
                                      'OB Registered Date', 'CSU Received Date', 'Remarks'])

      # Set the file location
      filelocation_main = withpath_df['Path'][i]

      # Provide the name of the range of the header to be extracted
      first_header = 'Adm. No'
      last_header = 'Remarks'

      wb = load_workbook(filelocation_main)
      ...
      if withpath_df['Type'][i] == 'Admission':
        ...
        for a in range(len(sheets)):
          ...
          if "dca" == sheets[a].lower():
            ...
            # Set the variable for use in next process
            dca_sheet = a
            wb.active = a
