...
def process_STPDM(guid, stepname, email, taskid, username, password, MAIN, INPUT, MAIN_FIXER, DESTINATION):
  ...
  if len(newfiles) > 1: #if only bat file exist means no file needed to rename, proceed to check in newfiles
    ...
    for j in range(len(newfiles2)):
      ...
      if(".bat" in tail.lower()) != True and os.path.exists(newfiles2[j]) == True:
        try:
          shutil.move(newfiles2[j], os.path.join(INPUT))
        except Exception as error:
          print("CANNOT MOVE:", newfiles2[j])
          print(error)
          pass
      else:
        pass
  ...
  def humanbytes(B):
    try:
      ...
      print("Error in humanbytes function")
  ...
  for e in range(len(extracted_clist['Filenames'])):
    ...
    if '.xlsx' in str(filename.lower()) or '.xls' in str(filename.lower()):
      ...
      try:
        ...
        print("Error in clean_file in workflow_process_stpdm.py")
        pass
      ...
      try:
        ...
        print("to_excel is not successful in workflow_process_stpdm.py")
        logging("STP-DM - Error in converting TXT to XLSX. Operation is aborted.", error, stpdm_base)

      if os.path.exists(head + "\\" + str(tail.split(".")[0]) + ".xlsx") == True:
        pass
      else:
        print("Error in to_excel in workflow_process_stpdm.py")

      split_flag = ''
      try:
        if str(humanbytes(os.path.getsize(head + "\\" + str(tail.split(".")[0]) + ".xlsx")).split(" ")[1]) == 'KB':
          split_flag = 'False'
          pass
        elif float(humanbytes(os.path.getsize(head + "\\" + str(tail.split(".")[0]) + ".xlsx")).split(" ")[0]) <= float(5):
          split_flag = 'False'
          pass
        else:
        ...
        print("Not spliting file to 400 rows in workflow_process_stpdm.py for", str(tail.split(".")[0]) + ".xlsx")
        pass

      try:
        shutil.move(extracted_clist['Filenames'][e], os.path.join(ORIGINAL, tail))
      except:
        print("Cannot move .txt file to original")
        pass

      try:
        if split_flag == 'True':
          shutil.move(head + "\\" + str(tail.split(".")[0]) + ".xlsx", os.path.join(ORIGINAL, tail))
          print("Original XLSX file has been moved.")
        else:
          print("Original XLSX file was not moved.")
          pass
      except:
        print("Cannot move .xlsx file to original")
        pass
  ...
  try:
    conn = MSSqlConnector()
    cur = conn.cursor()
    query = '''SELECT client_code, filename, fixer_filename, op_upload, ip_upload, status FROM [dm].[file_association]'''
    cur.execute(query)
    myfixer = pd.DataFrame(cur.fetchall(), columns=['client_code', 'filename', 'fixer_filename', 'op_upload', 'ip_upload', 'status'])
  except Exception as error:
  ...
  FIXER = ''
  extracted_clist = ''
  extracted_clist_copy = pd.DataFrame()
  ...
  if input_flag == True and new_files.empty != True:
    ...
    for j in range(len(myfixer)):
      ...
      if extracted_clist.empty != True:
        ...
        extracted_clist_copy = extracted_clist_copy.append(extracted_clist, ignore_index = True)
        extracted_clist_copy = extracted_clist_copy.reset_index()
        extracted_clist_copy = extracted_clist_copy.drop(columns = {'index'})
