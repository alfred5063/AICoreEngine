...
from workflow.workflow_process_admission import workflow_process_admission

@app.route('/process_admission', methods = ['POST'])
@login_required
def process_admission():
  try:
    ...
    # Call out main function from workflow_process_admission.py
    data = workflow_process_admission(myjson, guid, stepname, email, taskid, preview, username, password)
